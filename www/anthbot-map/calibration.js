...
export function adjustCalibration(calibration, action, amount = 1) {
  ...
  switch (action) {
    ...
    case "up": next.offsetY -= move; break;
    case "down": next.offsetY += move; break;
    case "wider": next.scaleX += scale; break;
    case "narrower": next.scaleX = Math.max(0.1, next.scaleX - scale); break;
    case "taller": next.scaleY += scale; break;
    case "shorter": next.scaleY = Math.max(0.1, next.scaleY - scale); break;
    case "rotate-left": next.rotation -= rotate; break;
    case "rotate-right": next.rotation += rotate; break;
    case "rotate-left-large": next.rotation -= rotateLarge; break;
    case "rotate-right-large": next.rotation += rotateLarge; break;
...
export function cardToYaml(
  ...
  if (config.map_only === true || config.mapOnly === true) lines.push("map_only: true");
  if (config.transparent_background === true || config.transparentBackground === true) lines.push("transparent_background: true");
  if (config.robot_image || config.robotImage) lines.push(`robot_image: ${quoteYaml(config.robot_image || config.robotImage)}`);
  if (Number.isFinite(Number(config.robot_size || config.robotSize))) lines.push(`robot_size: ${formatNumber(config.robot_size || config.robotSize)}`);
  if (Number.isFinite(Number(config.robot_image_rotation ?? config.robotImageRotation))) lines.push(`robot_image_rotation: ${formatNumber(config.robot_image_rotation ?? config.robotImageRotation)}`);
  if (config.robot_heading_source || config.robotHeadingSource) lines.push(`robot_heading_source: ${quoteYaml(config.robot_heading_source || config.robotHeadingSource)}`);
  if (Number.isFinite(Number(config.robot_heading_offset ?? config.robotHeadingOffset))) lines.push(`robot_heading_offset: ${formatNumber(config.robot_heading_offset ?? config.robotHeadingOffset)}`);
  if (Number.isFinite(Number(config.robot_mowing_heading_offset ?? config.robotMowingHeadingOffset))) lines.push(`robot_mowing_heading_offset: ${formatNumber(config.robot_mowing_heading_offset ?? config.robotMowingHeadingOffset)}`);
  if (config.show_mowed_path === false) lines.push("show_mowed_path: false");
  if (config.show_decoded_boundary === false || config.showDecodedBoundary === false) lines.push("show_decoded_boundary: false");
  if (config.show_zones === false || config.showZones === false) lines.push("show_zones: false");
  if (config.show_no_go_zones === false || config.showNoGoZones === false) lines.push("show_no_go_zones: false");
  if (config.show_no_go_labels === false || config.showNoGoLabels === false) lines.push("show_no_go_labels: false");
  if (config.show_legacy_boundary === true || config.showLegacyBoundary === true) lines.push("show_legacy_boundary: true");
  if (config.mowed_path_color || config.mowedPathColor) lines.push(`mowed_path_color: ${quoteYaml(config.mowed_path_color || config.mowedPathColor)}`);
  if (config.mowed_path_source || config.mowedPathSource) lines.push(`mowed_path_source: ${quoteYaml(config.mowed_path_source || config.mowedPathSource)}`);
  if (Number.isFinite(Number(config.mowed_path_width ?? config.mowedPathWidth))) lines.push(`mowed_path_width: ${formatNumber(config.mowed_path_width ?? config.mowedPathWidth)}`);
  if (config.fit) lines.push(`fit: ${quoteYaml(config.fit)}`);
...
if (typeof customElements !== "undefined") {
  customElements.whenDefined("anthbot-map-card").then(() => {
    ...
    proto.updateMowingProgressStatus = function (...args) {
      ...
      const finished = ["returningtodock", "backtodock", "returntodock", "docking", "charging", "charge", "docked", "standby", "idle"].some((value) => status.includes(value));
