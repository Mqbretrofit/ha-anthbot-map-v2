...
class _CloudErrorReporter:
    ...
    def handle_event(self, event: CloudApiErrorEvent) -> None:
        ...
        # Reporting is best-effort background work and must never extend Home
        # Assistant's startup phase. ConfigEntry owns and cancels the task on
        # unload, matching the developer-agent lifecycle.
        self.entry.async_create_background_task(
            self.hass,
            self._async_send_report(installation_id=installation_id, event=event),
            f"anthbot_cloud_error_report_{self.entry.entry_id}",
        )
...
async def async_register_cloud_error_reporting(
    ...
    # Keep reporter instances alive for as long as their callbacks are
    # registered. The bound callbacks also retain them, but the explicit list
    # documents the intended lifecycle and prevents accidental refactors from
    # making them ephemeral.
