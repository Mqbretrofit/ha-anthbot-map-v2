...
def _status_value(data: dict[str, Any]) -> Any:
    ...
    M5/M9 devices were found (see m_series_compat.py) to report live status
    on the same *property* shadow as Genie, so this normally resolves on the
    first pass exactly like Genie does. The "mode" key and the
    "_service_reported" fallback are kept defensively for any M-series unit
    or firmware that turns out to behave differently -- they are no-ops on
    every payload shape observed so far.
    """
