"""Temporary local-only M5/M9 device-log/media diagnostic probe.

The official app exposes ``log_switch`` and ``log_upload`` but no camera viewer.
This test helper reproduces only those observed commands, captures short-lived
property/service shadow changes, redacts credentials/location data, and stores a
JSON report under ``/config/www/anthbot-device-log-probes``. Nothing is uploaded
by this helper.
"""
...
_SERVICE = "probe_m9_device_log_media_test"
_EVENT = "anthbot_device_log_media_probe_ready"
...
def _is_m_series(value: object) -> bool:
...
async def _shadow(
    ...
    """Read the actual named shadow, bypassing M-series service aliases."""
    try:
...
async def _publish_exact(
    ...
    """Publish the exact app-style desired object through the live MQTT socket."""
...
async def _run(coordinator: Any) -> dict[str, Any]:
    ...
    if not _is_m_series(getattr(coordinator.device, "model", None)):
        raise AnthbotGenieApiError("Device-log/media probe is M5/M9-only")
    ...
    suffix = re.sub(r"[^A-Za-z0-9_-]", "_", serial)[-8:] or "mower"
    filename = f"anthbot_log_media_probe_{suffix}_{finished:%Y%m%d_%H%M%S}.json"
    file_path = Path(hass.config.path("www", "anthbot-device-log-probes", filename))
    report = {
        "schema": "anthbot-device-log-media-probe-v1",
    ...
    _LOGGER.warning("ANTHBOT device-log/media probe completed: %s", result)
    return result


def _loaded_m_series(hass: Any) -> list[Any]:
    result: list[Any] = []
    ...
    for value in loaded.values():
        for coordinator in value if isinstance(value, list) else []:
            if _is_m_series(getattr(getattr(coordinator, "device", None), "model", None)):
...
def _ensure_service(hass: Any) -> None:
    ...
    async def handle(call: Any) -> None:
        ...
        candidates = _loaded_m_series(hass)
        if serial:
            candidates = [c for c in candidates if str(c.client.serial_number) == serial]
        if not candidates:
            raise AnthbotGenieApiError("No loaded M5/M9 mower matches the request")
        if len(candidates) != 1:
            raise AnthbotGenieApiError("Multiple M5/M9 mowers loaded; pass serial_number")
        ...
        try:
            await hass.services.async_call(
                ...
                "create",
                {
                    "title": "ANTHBOT M9 log/media probe",
                    "message": f"JSON elkészült: {result['url']}",
                    "notification_id": "anthbot_m9_log_media_probe",
                },
                blocking=False,
            )
    ...
    hass.async_create_background_task(expire(), "anthbot_m9_log_probe_service_expiry")


def install_m_series_log_probe() -> None:
    """Register the temporary service when an M5/M9 coordinator is constructed."""
    global _INSTALLED
    if _INSTALLED:
        return
    _INSTALLED = True
