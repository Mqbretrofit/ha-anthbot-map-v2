"""Shared family policy for M5/M9/M9 Pro model modules."""
...
def path_points_with_metadata(decoded: dict[str, Any]) -> list[dict[str, Any]]:
...
def bootstrap_existing_path(coordinator: Any) -> list[dict[str, Any]]:
    ...
    for key in ("_path_definition", "path_definition"):
        ...
        if isinstance(definition, dict):
            points = path_points_with_metadata(definition)
            if points:
                return points
...
def accumulate_path(coordinator: Any, decoded: dict[str, Any], packet_points: list[dict[str, float]]) -> list[dict[str, Any]]:
    del packet_points
    packet = path_points_with_metadata(decoded)
    path_id = decoded.get("path_id")
    previous_path_id = getattr(coordinator, "_m_series_path_id", None)
    try:
        start = max(0, int(decoded.get("start", 0) or 0))
    except (TypeError, ValueError):
        start = 0
    accumulator = bootstrap_existing_path(coordinator)
    ...
    return accumulator[-PATH_MAX_POINTS:]


def map_candidates(property_state: dict[str, Any]) -> tuple[str, ...]:
    candidates: list[str] = []
    ...
    candidates.extend(MAP_CANDIDATES)
