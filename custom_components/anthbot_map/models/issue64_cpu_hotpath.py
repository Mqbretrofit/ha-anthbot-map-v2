        ...
        if isinstance(current, dict):
            ...
            for key, item in current.items():
                if info is None and key in coordinator_module._HISTORY_INFO_KEYS:
                    info = item
                if url is None and key in coordinator_module._HISTORY_PATH_URL_KEYS and isinstance(item, str) and item.startswith(("http://", "https://")):
                    url = item
            for item in reversed(tuple(current.values())):
                if isinstance(item, (dict, list)):
                    stack.append(item)
                elif url is None and isinstance(item, str) and item.startswith(("http://", "https://")) and any(part in item.lower() for part in ("path", "history", "record")):
                    url = item
            ...
            for item in reversed(current):
                if isinstance(item, (dict, list)):
                    ...
                    url = item
        elif url is None and isinstance(current, str) and current.startswith(("http://", "https://")) and any(part in current.lower() for part in ("path", "history", "record")):
            url = current
    return info, url
...
def _segment_misses_box(first: tuple[float, float], second: tuple[float, float], box: tuple[float, float, float, float]) -> bool:
    return max(first[0], second[0]) < box[0] or min(first[0], second[0]) > box[2] or max(first[1], second[1]) < box[1] or min(first[1], second[1]) > box[3]
...
def _evaluate_path_no_go(path_points: Any, data: dict[str, Any], *, path_id: Any = None) -> dict[str, Any]:
    ...
    return {"source": "m_series_assembled_path", "path_id": path_id, "crossing_detected": bool(inside_path_indices or boundary_crossings), "checked_point_count": valid_point_count, "checked_segment_count": segment_count, "no_go_zone_count": len(zones), "points_inside": len(inside_path_indices), "boundary_crossings": boundary_crossings, "traversals": traversals, "zone_ids": affected_zone_ids, "last_crossing": last_crossing, "zones": zone_results}
