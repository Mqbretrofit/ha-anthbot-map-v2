...
class VoicePackSupportTests(unittest.TestCase):
    ...
    def test_paid_voices_are_visible_before_purchase_and_locked(self) -> None:
        ...
        self.assertIn('"voice_store_locked_pack_ids"', select)
        self.assertIn("pack.store_pack_id", select)
        ...
        for path in (
            ...
            self.assertIn("attrs.voice_store_locked_pack_ids", card)
            self.assertIn('"/api/anthbot/store/direct-checkout"', card)
            self.assertIn('directUrl.searchParams.set("pack_id", packId)', card)
            self.assertIn("checkoutUrl || voiceStoreUrl", card)
