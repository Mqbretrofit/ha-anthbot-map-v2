...
# Anthbot Map v2.2.0 – Magyar változáslista

Ez a stabil kiadás a stabil `2.0.0` óta elkészült összes változást tartalmazza,
beleértve a terepen kipróbált `2.2.1-beta.48` funkciókészletet, valamint a végső
valós idejű szegélykezelési és HACS-telepítési javításokat.

## A v2.2.0 valós idejű szegélykezelése és HACS-telepítése

- A szegélydefiníció azonnal frissül, amikor a robot új `ridable_area_time`
  értéket küld, így a hivatalos ANTHBOT alkalmazásban végzett módosítások is
  valós időben megjelennek.
- A vágási magasság vagy szegélyátfedés mentése megvárja, amíg a robot közzéteszi
  és eltárolja a kért értékeket; többé nem csak egy helyi, előre feltételezett
  érték jelenik meg.
- A szerkesztő automatikusan frissíti az értékeit az újonnan letöltött,
  alkalmazáskompatibilis szegélydefinícióból.
- A számszerű azonosítóval rendelkező érvényes szegély akkor is szerkeszthető,
  ha a geometriai pontlistája üres; egyes robottérképek ezt az aktív formátumot
  használják.
- Ha csak egy szegély van, a neve egyszerűen **Szegély**, nem pedig egy elavult
  felhőnév, például **Szegély 2**.
- A `ridable_area_time` megjelenik a térképérzékelő attribútumai között a
  diagnosztikához és a valós idejű felületi frissítéshez.
- Storage módú Lovelace esetén az integráció automatikusan létrehozza vagy
  frissíti a JavaScript-modul erőforrását. A meglévő megfelelő erőforrást
  újrahasználja, minden más irányítópult-erőforrást változatlanul hagy.
- YAML erőforrásmódban továbbra is kézzel kell felvenni az erőforrást.

## A 2.0.0 óta bekerült korábbi szegélybeállítási javítások
...
- Sikeres mentés után az új vágási magasság és szegélyátfedés a robot által
  visszaigazolt felhődefinícióból töltődik vissza, ezért a szerkesztő újbóli
  megnyitásakor nem jelenik meg elavult gyorsítótárazott érték.
- Ugyanez az adatforrás- és valós idejű frissítési javítás bekerült az Anthbot
  Map Cardba és a Garden Map Card v164 verziójába is.
- A számszerű azonosítóval rendelkező érvényes szegélyrekordok használható
  csúcspontok nélkül is szerkeszthetők.
...
- A robot és a mérőnyíl együtt mozog; a távolság mérése a szegélytől a robot
  széléig történik.
- A számszerű azonosítóval rendelkező szegélyek csúcspontok nélkül is
  támogatottak; az azonosító nélküli törölt vagy hibás rekordok nem jelennek meg.
...
- Mind a 74 automatikus Python regressziós teszt sikeres.
